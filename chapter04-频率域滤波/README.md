...
**结论**：频率域样本间的间隔与空间样本间的间距及样本数成反比。

#### 4.6.2 平移和旋转

**结论**：

1）**平移**：空间域乘以指数等价于频域移动；空间域移动等价于频域乘以指数；

2）**旋转**：空间域旋转多少角度，频域就旋转多少角度。

#### 4.6.3 周期性

**结论**：二维傅里叶变换及其反变换都是无限周期的。需要理解式（4.6-8）。

#### 4.6.4 对称性

**小结**：表4.1列出了DFT的对称性和相关的性质，这些性质在数字图像处理中很有用。

#### 4.6.5 傅里叶谱和相角

**结论**：由式（4.6-21）可知，直流项决定图像的评价灰度，因此将其置0会把输出图像的平均灰度减小为0。

**直流（dc）成分**：式（4.6-22）的值被称为直流成分。

**例 4.13 一个简单函数的二维傅里叶谱**：

1）频谱的原点周围包含了最高值；

2）而且由于频谱的周期性，4个角都包含有类似的高值；

3）为了易于观察处理，可以中心化频谱，在计算DFT之前简单使用-1的x+y次方乘以空间域图像；

4）由于直流项支配着谱的值，在显示的图像中，其他灰度的动态范围被压缩了，可以通过如3.22节中的对数变换来使得频谱细节得到提升；

5）空间域的移动对频谱无影响，空间域旋转多少度相应的频域就旋转多少度。

**结论**：相角是各个正弦成分关于原点的位移的度量。

**例 4.14 傅里叶谱和相角的进一步说明**：相角虽然直观上难以理解，但是其包含了图像形状信息。幅度谱包含了图像的灰度信息。具体见图4.27。

#### 4.6.6 二维卷积定理

**小结**：

1）空域卷积对应频域乘积；空域乘积对应频域卷积；

2）基于DFT的卷积可能会产生频域缠绕，解决这一问题的方法见式（4.6-26）；

3）基于二维DFT的卷积避免频域缠绕的见式（4.6-27）到（4.6-31）， P.S 最好将P和Q选为满足上面方程的最小偶整数。如果两个阵列大小相同，则意味着P和Q选为该阵列大小的2倍。

#### 4.6.7 二维离散傅立叶变换性质的小结

**小结**：本小节主要展示了关于二维离散傅立叶变换性质的3张表。

### 4.7 频率域滤波基础

#### 4.7.1 频率域的其他特性

**小结**：频率域和空间域中图像的亮度变化可以建立直观上的联系。
